    ...
    <>
      ...
      <section id="pricing" className="relative py-16 md:py-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="mb-10">
            <StateMap onStateSelect={handleStateSelect} selectedState={selectedState} />
          </div>
            ...
            <div className="relative">
              ...
              {showStateDropdown && (
                <div className="absolute left-0 right-0 mt-2 bg-black/90 backdrop-blur-2xl border border-white/10 rounded-xl shadow-2xl max-h-60 overflow-y-auto z-50">
                  {STATES.map((state) => (
                    <button key={state.id} onClick={() => { setSelectedState(state.slug); setShowStateDropdown(false) }} className={`w-full flex items-center justify-between px-4 py-3 text-sm hover:bg-white/5 transition ${selectedState === state.slug ? 'text-blue-300 bg-blue-500/10' : 'text-white/60'}`}>
                      <span className="font-light">{state.name}</span>
                      <span className="text-white/20 text-xs">${state.filingFee} filing fee</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="max-w-3xl mx-auto mb-10 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-full text-white/40 text-xs font-light tracking-wide">
              ...
              State filing fees vary and are separate from LIMBIZ service fees
            </div>
          </div>
          ...
          <div id="pricing-packages" className="grid md:grid-cols-3 gap-6">
            {packages.map((pkg, idx) => {
              ...
              return (
                <motion.div key={pkg.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.08, duration: 0.4 }} className={`relative rounded-2xl p-6 transition-all duration-500 ${isPopular ? 'bg-gradient-to-b from-blue-500/10 to-purple-500/10 border-2 border-blue-400/30 shadow-lg shadow-blue-500/5' : 'bg-white/5 backdrop-blur-sm border border-white/10 hover:border-blue-400/20'}`}>
                  {isPopular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-blue-400 to-blue-300 text-white text-[10px] font-bold uppercase tracking-wider px-4 py-1 rounded-full shadow-lg shadow-blue-500/20">Most Popular</div>}
                  <div className="text-center mb-6">
                    <h3 className="text-xl font-light text-white/90 tracking-tight">{pkg.name}</h3>
                    <p className="text-white/30 text-sm mt-1 font-light">{pkg.description}</p>
                  </div>
                  <div className="space-y-2 mb-6">
                    <div className="flex justify-between items-center p-2 bg-white/5 rounded-lg border border-white/5"><span className="text-white/40 text-xs font-light">State Filing Fee</span><span className="text-white/80 font-light">${currentStateData?.filingFee || '—'}</span></div>
                    <div className="flex justify-between items-center p-2 bg-white/5 rounded-lg border border-white/5"><span className="text-white/40 text-xs font-light">LIMBIZ Service Fee</span><span className="text-white/80 font-light">${pkg.limbiZFee}</span></div>
                    <div className="flex justify-between items-center p-3 bg-blue-500/10 rounded-lg border border-blue-400/20"><span className="text-blue-300/60 text-xs font-light">Estimated Total</span><span className="text-blue-300 font-bold text-xl">${total}</span></div>
                  </div>
