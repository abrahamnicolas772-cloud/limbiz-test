...
export default function Navbar() {
  ...
  const submenus = [
    { id: 'formation', title: 'Business Formation', links: ['LLC Filing', 'EIN Registration', 'Operating Agreement', 'DBA / Fictitious Name', 'S-Corporation Election', 'LLC Amendments', 'Foreign Qualification', 'Business Reinstatement', 'Business Dissolution'] },
    { id: 'setup', title: 'Business Setup', links: ['Business Address', 'Registered Agent', 'Business Email', 'Google Business Profile', 'Licenses & Permits', 'Sales Tax Permit', 'D-U-N-S Number'] },
    { id: 'growth', title: 'Growth & Compliance', links: ['Business Credit Building', 'Funding Assistance', 'Tax Filing Assistance', 'Annual Report Filing', 'Compliance Monitoring'] },
    { id: 'international', title: 'International Services', links: ['ITIN Application', 'Intl Business Formation', 'Foreign-Owned LLC', 'EIN for Foreign Owners', 'U.S. Business Address', 'Registered Agent'] },
    { id: 'protection', title: 'Protection & Digital', links: ['Trademark Application', 'Copyright Registration', 'Business Branding', 'Logo & Brand Identity', 'Website Development', 'Domain Registration', 'E-commerce Setup', 'Social Media Setup', 'Digital Business Card'] },
  ]
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-7">
            ...
            <div className="relative">
              ...
              {servicesOpen && (
                <div className="absolute left-0 top-full mt-2 w-72 bg-black/95 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl p-3 z-50 max-h-[70vh] overflow-y-auto">
                  {submenus.map((menu) => (
                    <div key={menu.id} className="mb-1">
                      ...
                      {openSubmenu === menu.id && (
                        <div className="ml-6 mt-1 space-y-0.5 border-l border-white/10 pl-3">
                          {menu.links.map((label) => (
                            <Link key={label} href="/services" onClick={() => { setServicesOpen(false); setOpenSubmenu(null) }} className="block px-3 py-1.5 text-white/50 hover:text-white hover:bg-white/5 rounded-lg text-xs transition">
                              {label}
                            </Link>
                          ))}
                        </div>
              ...
              {ecosystemOpen && (
                <div className="absolute left-0 top-full mt-2 w-56 bg-black/95 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl p-3 z-50">
                  <ul className="space-y-1">
                    ...
                    <li><Link href="/books" onClick={() => setEcosystemOpen(false)} className="block px-3 py-2 text-white/60 hover:text-white hover:bg-white/5 rounded-lg text-sm transition">Books & Guides</Link></li>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {user ? (
              ...
              <>
                <Link href="/login" className="px-4 py-2 text-sm text-white/70 hover:text-white transition">Login</Link>
                <Link href="/register" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-full text-white text-sm font-medium transition">Sign Up</Link>
              </>
            )}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-black/95 border-t border-white/10 py-4">
          <div className="flex flex-col items-center gap-3">
            ...
            <Link href="/books" onClick={()=>setIsOpen(false)} className="text-white/70 text-sm">Books & Guides</Link>
